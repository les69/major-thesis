...
issue we had to deal with:
 %%First of all Swift or Objective-C is not
%%meant for building web services, therefore there are no official libraries for
%starting and handling HTTP requests. Second, the architecture for mobile applications
%doesn't fit well with the standard daemon running in the background. Third, the
%\textit{HomeKit} accessories are accessible only from a specific component, \textit{UIView},
%which means the server has to be started in one of these views.
